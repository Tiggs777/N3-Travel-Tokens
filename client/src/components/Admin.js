...
const Admin = () => {
  ...
  const [activeTab, setActiveTab] = useState(location.state?.activeMenu || 'tokensView'); // Default to tokensView
  ...
  const [deleteAction, setDeleteAction] = useState(null); // Store the delete function to confirm
  const [isSessionExpired, setIsSessionExpired] = useState(false); // Session timeout state
  const [tokenViewMode, setTokenViewMode] = useState('list'); // 'list' or 'tiles' for tokensView
  const [selectedToken, setSelectedToken] = useState(null); // For token detail pop-up
  const [userViewMode, setUserViewMode] = useState('list'); // For usersView toggle
  const [travelPackageViewMode, setTravelPackageViewMode] = useState('list'); // For travelPackageView toggle
  const [selectedUser, setSelectedUser] = useState(null); // For user detail pop-up
  const [selectedTravelPackage, setSelectedTravelPackage] = useState(null); // For travel package detail pop-up
  ...
  useEffect(() => {
    ...
    deleteButtons.forEach(btn => console.log('Button:', btn.textContent, 'Width:', btn.offsetWidth));
  }, [activeTab]);
...
const fetchTravelPackageGroups = async () => {
  ...
  return (
    <div className="admin-container">
      ...
      <div className="admin-tabs">
        ...
        {/* Removed travelView tab group entirely */}
  ...
  <h2>View Users</h2>
  <div className="view-toggle">
    <button className={userViewMode === 'list' ? 'tab active' : 'tab'} onClick={() => setUserViewMode('list')}>
      List
    </button>
    <button className={userViewMode === 'tiles' ? 'tab active' : 'tab'} onClick={() => setUserViewMode('tiles')}>
      Tiles
    </button>
  </div>
  ...
  {userViewMode === 'list' ? (
    ...
    <div className="user-tiles">
      {users.map(user => (
        <div key={user.id} className="user-tile" onClick={() => setSelectedUser(user)}>
          <input
            type="checkbox"
            checked={selectedUsers.includes(user.id)}
            onChange={() => handleSelectUser(user.id)}
    ...
    <>
      {(group.user_ids || []).map((userId, index) => {
        ...
        {activeTab === 'travelPackageView' && (
         ...
         <div className="view-toggle">
           <button className={travelPackageViewMode === 'list' ? 'tab active' : 'tab'} onClick={() => setTravelPackageViewMode('list')}>
             List
           </button>
           <button className={travelPackageViewMode === 'tiles' ? 'tab active' : 'tab'} onClick={() => setTravelPackageViewMode('tiles')}>
             Tiles
           </button>
         </div>
         ...
         {travelPackageViewMode === 'list' ? (
           <table className="travel-table">
             ...
             <tbody>
               {travelPackages.map(pkg => (
                 <tr key={pkg.id} onClick={() => setSelectedTravelPackage(pkg)} style={{ cursor: 'pointer' }}>
                   <td>
                     <input
                       type="checkbox"
           ...
           <div className="travel-package-tiles">
             {travelPackages.map(pkg => (
               <div key={pkg.id} className="travel-package-tile" onClick={() => setSelectedTravelPackage(pkg)}>
                 <input
                   type="checkbox"
                   checked={selectedPackages.includes(pkg.id)}
                   onChange={() => handleSelectPackage(pkg.id)}
...
>
  {selectedTravelPackage && (
    <div className="travel-package-detail-card"> {/* New class for travel package details */}
      <img src={selectedTravelPackage.image_url} alt={selectedTravelPackage.name} className="token-image" />
      <h3>{selectedTravelPackage.name}</h3>
      <p><strong>ID:</strong> {selectedTravelPackage.id}</p>
      <p><strong>Price:</strong> {selectedTravelPackage.price}</p>
      <p><strong>Description:</strong> {selectedTravelPackage.description}</p>
      <div className="modal-button-group">
